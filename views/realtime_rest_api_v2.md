...
|商用版   |      开发版    |
|----------|---------------|
| 每应用每分钟 1800（默认）/ 9000（最大）次 | 每应用每分钟 120 次  |

所有接口共享额度。超过额度限制后一分钟内 LeanCloud 会拒绝请求持续返回 429 错误码，一分钟后会重新处理请求。

商用版应用默认上限为 1800 次，可以在[控制台 > 消息 >  即时通讯 > 服务阈值 > 普通消息 API 调用频率上限][dashboard-rtm-limit]修改（最高可设置为 9000 次）。
修改上限后，按照每日调用频率峰值实行阶梯收费，阶梯单元 1800 次/分钟，第一阶（峰值在 1800 次/分钟以下）免费，第二阶收费 10 元（国际版为 3 美元），之后每阶收费递增 10 元（3 美元），如下表所示：
