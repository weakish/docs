...
- 左栏按照服务类型做了一级分类，包括存储（结构化数据、用户、文件、LiveQuery、应用内搜索、导入导出、用量统计），云引擎（网站托管、云函数、定时任务、LeanCache、LeanDB），消息（即时通讯、推送、短信），游戏（多人在线对战、ClientEngine、排行榜），组件（用户反馈、社交），设置（基本信息、应用 Keys、安全中心、域名绑定、邮件模板、协作管理、风险检测、操作日志、数据导出与恢复）。
- 一级分类下还有二级菜单，详情见以下章节。
...
除了用量统计页面以外，如果你在结构化数据页面的 Class 名称旁看到秒表图标，就表示这个 Class 过去 7 天内有慢查询。
...
请进入 [控制台 > 账号设置 > 域名绑定](/dashboard/settings.html#/setting/domainbind)，按照步骤提示操作即可。

在云引擎网站托管项目的测试和开发阶段，你也可以暂时使用我们提供的云引擎开发域名。可以到 **设置 > Web 主机域名** 里面申请。
