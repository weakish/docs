...
- `区域 2` 按照服务类型，将应用数据做了一级分类。这里分成了存储（包括结构化数据存储、文件存储、云引擎、离线数据分析等功能）、消息（包括即时通信、消息推送和短信服务三种）、分析（应用/用户分析结果展示）、组件（其它扩展组件，如用户反馈、应用内搜索、社交组件等）四个大类，也增加了应用设置菜单。
...
- 「安全中心」：可以给数据存储、短信发送、推送、即时通信等服务进行单独开关设置，同时也可以为 web 应用在使用 js sdk 的时候设置安全域名。
