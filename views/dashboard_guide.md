...
- 左栏按照服务类型做了一级分类，包括存储（结构化数据、用户、文件、LiveQuery、应用内搜索、导入导出、用量统计），云引擎（网站托管、云函数、定时任务、LeanCache、LeanDB），消息（即时通讯、推送、短信），游戏（多人在线对战、ClientEngine、排行榜），设置（基本信息、应用 Keys、安全中心、域名绑定、邮件模板、协作管理、风险检测、操作日志、数据导出与恢复）。
- 一级分类下还有二级菜单，详情见以下章节。
