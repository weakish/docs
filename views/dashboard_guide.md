...
#### 云引擎中自定义二级域名
云引擎默认为每个应用提供了一个二级域名 `<应用的域名>.{{engineDomain}}`，允许开发者将 web 应用部署到该域名之下运行，支持静态资源和动态请求服务。这个二级域名是可以修改的，你可以到 **存储 > 云引擎 > 设置** 里面设置自定义的子域名。
...
#### 设置 iOS 离线推送配置项
在 **消息 > 实时消息 > 设置** 菜单下，可以打开／关闭「[iOS 用户离线推送](./realtime-guide-intermediate.html#离线推送通知)」的开关，也可以设置推送使用的证书和推送文本（什么都不设置即表示关闭「iOS 用户离线推送」功能）。
...
- 「安全中心」：可以给数据存储、短信发送、推送、即时通讯等服务进行单独开关设置，同时也可以为 web 应用在使用 js sdk 的时候设置安全域名。
...
注意，`createdAt` 和 `updatedAt` 属于保留字段，导出数据中直接编码为 UTC 时间戳字符串，和普通 Data 类型字段的编码方式不同。这一点与通过 REST API 查询这两个字段的返回结果是一致的。
