...
// 你可以在所有云引擎代码（包括云函数也包括网站托管中的自定义路由；包括主要分组也包括其他分组）使用 Cloud.enqueue
...
- **使用单独的分组运行任务** 我们有计划为用户提供一个「在独立分组中运行定时任务」的选项，开启这个选项后会自动创建一个特殊分组，所有定时任务都在这个分组中运行，这个分组不需要单独部署，会自动同步主要分组中的代码。
- **与多分组配合使用** 我们我们只有主要分组支持云函数，但你可以在任意分组来进行入队操作（`Cloud.enqueue`），最终都会执行到主要分组上的云函数。同时我们有计划在所有分组上都支持云函数，但优先级较低。
...
### deliveryMode 具体影响哪些情况？

deliveryMode 用于在以下几种不确定任务是否已经执行或是否应该执行的情况下，来决定是否重试：
