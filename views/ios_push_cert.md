...
2. 点击 **Membership**
  ...
  ![Membership](images/ios_cert_v2/find_team_id.png)
...
{{ docs.alert("如果目标推送设备中有使用版本低于 v4.2.0 的 iOS SDK，请在调用推送接口发推送时主动带上 topic 参数。用法请参考 [推送消息接口](push_guide.html#推送消息)。") }}
...
一种解决方法是替换过期的证书，另一种方法是在 query 条件中通过 `deviceType` 字段明确指定 `_Installation` 表中设备类型为非 iOS 设备来推送，方法参见 [消息推送总览 · 通过查询条件发推送](push_guide.html#通过查询条件发推送)。
