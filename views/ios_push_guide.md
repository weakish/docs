...
通过 [REST API](push_guide.html#notification) 和控制台推送消息时可以通过 `prod` 参数指定环境。
通过 SDK 使用 `Push` 发起推送默认使用生产环境，如果要切换到测试环境，方式如下：
...
注意，SDK 只能指定发送给哪种环境的应用，无法切换接收生产环境或测试环境的推送，接收哪种环境的推送完全由 App 的分发方式决定。
