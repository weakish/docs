...
为了保证获取聊天记录的安全性，可以开启签名认证 **控制台 > 消息 > 即时通讯 > 设置 > 即时通讯选项** 中的 **聊天记录查询启用签名认证**。了解更详细的签名规则请参考 [聊天签名方法](realtime-guide-senior.html#安全与签名)。签名参数仅在开启对应的选项后有效，如果没有开启选项，就不需要传签名参数。
...
**控制台 > 消息 > 即时通讯 > 设置 > 即时通讯选项** 中的「允许通过 SDK 编辑消息」和「允许通过 SDK 撤回消息」仅影响 SDK，对本接口无影响。
...
如果你在应用中使用了我们内部定义的 [富媒体消息格式](./realtime_v2.html#消息（Message）)，在发送消息时 **message** 字段需要遵守一定的格式要求，下文 [富媒体消息格式说明](./realtime_v2.html#富媒体消息) 中将详细说明。
...
本接口可以让使用了 [AV.User](rest_api.html#用户-1) 的应用方便快捷地实现登录认证。登录认证默认关闭，可以进入 **控制台 > 消息 > 即时通讯 > 设置 > 即时通讯选项**，选中 **登录启用签名认证**。
