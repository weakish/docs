...
- `stg-` 开头的自定义域名（例如 `stg-xxx.example.com`）会被自动地绑定到预备环境。
...
根据工信部规定，在 LeanCloud 绑定的文件域名、云引擎域名、API 域名，需要在 LeanCloud 处备案或者接入备案：

1. 主域名在 LeanCloud 备案，使用子域名在 LeanCloud 绑定文件域名、云引擎域名、API 域名不需要额外备案或接入备案。例如，假设你的主域名 `example.com` 是在 LeanCloud 处备案的，那么绑定 `file.example.com` 为文件域名，绑定 `web.example.com` 为云引擎域名，绑定 `api.example.com` 为 API 域名，都不需要额外备案或接入备案，直接在控制台绑定即可。
...
3. 如果你的主域名没有备案，那么主域名本身及其子域名均无法在控制台绑定，需要先在 LeanCloud 或其他云服务商处办理备案。管局审核通过后，方可在 LeanCloud 控制台绑定。

### 新增备案

如果你的主域名没有备案（没有 ICP 备案号），我们建议你在 LeanCloud 新增备案。

商用版应用请进入 [应用控制台 > 账号设置 > 域名备案](/dashboard/settings.html#/setting/domainrecord)，按照步骤填写资料，并根据控制台提示进行新增备案。

没有商用版应用的用户，可以通过以下几种方式新增备案：

1. 升级应用为商用版，通过控制台新增备案。
2. 如果你同时也是 LeanCloud 底层 IaaS 服务商（华北节点为 UCloud）的用户，那么也可以自行在底层 IaaS 服务商新增备案。备案通过后，等价于在 LeanCloud 新增备案。
3. 如果只需要绑定文件域名，可以在其他云服务商处新增备案，然后直接在 LeanCloud 控制台绑定文件域名，因为绑定文件域名不需要接入备案。注意，这仅仅适用于只需要绑定文件域名的少数用户。对于大部分用户而言，因为需要绑定 API 域名或云引擎域名，在其他云服务商处新增备案之后，仍然需要在 LeanCloud 处接入备案，这意味着需要重复提交许多类似的资料，经过两次审核周期，由管局先后审核两次，没有意义，不如直接在 LeanCloud 新增备案。

### 接入备案

工信部规定，接入多个云服务商时，需要进行接入备案。
因此，如果你的主域名已在别处备案（有 ICP 备案号），并且需要绑定 API 域名或云引擎域名，那么你需要在 LeanCloud 接入备案。

同一个网站（应用）可以有多个接入商，不会冲突。
...
需要接入备案的域名请先在 LeanCloud 控制台完成绑定，再办理接入备案。
办理接入备案期间域名、服务可以正常使用。

商用版应用如需接入备案，请进入 [应用控制台 > 账号设置 > 域名备案](/dashboard/settings.html#/setting/domainrecord)，按照步骤填写资料，并根据控制台提示进行接入备案。
