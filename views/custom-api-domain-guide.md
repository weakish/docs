...
| 域名种类 | 涉及服务 | 备案 | 在 LeanCloud 备案或接入备案 | SSL | 裸域名 | A 记录 | 绑定目标 | 可绑定多个域名 | 国际版 | 
| - | - | - | - | - | - | - | - | - | - |
| 文件域名 | 文件服务 | 必须 | 可选 | 可选 | 不支持 | 不支持 | 应用 | 否 | 不支持 |
| 云引擎域名 | 云引擎 **网站托管** | 必须 | 必须 | 可选 | 支持 | 支持 | 分组 | 是 | 支持 |
| API 域名 | 存储、即时通讯、短信、推送、**云函数** | 必须 | 必须 | 必须 | 不支持 | 不支持 | 应用 | 是 | 不支持 |
