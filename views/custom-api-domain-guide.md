...
1. API 域名需要启用 SSL，绑定时会自动申请 SSL 证书，当然你也可以自行上传证书。华东节点自动申请 SSL 证书的功能尚未开发完成，目前华东节点控制台暂时可以选择不启用 SSL。
...
（华东节点前往[应用控制台 > 存储 > 设置 >  自定义 API 服务域名](https://tab.leancloud.cn/storage.html?appid={{appid}}#/storage/conf) 绑定 API 域名。） 
