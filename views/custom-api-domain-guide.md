...
根据法律法规的规定，在 LeanCloud 绑定的文件域名、云引擎域名、API 域名，需要在 LeanCloud 处备案或者接入备案：
...
同一个网站（应用）可以有多个接入商，不会冲突。需要接入备案的域名请先在控制台完成绑定，再办理接入备案。办理接入备案期间域名、服务可以正常使用。

商用版应用的域名如果需要在 LeanCloud 处备案或者接入备案，请进入 [应用控制台 > 账号设置 > 域名备案](/dashboard/settings.html#/setting/domainrecord)，按照步骤填写资料。
