...
[控制台 > 云引擎 > 统计](/cloud.html?appid={{appid}}#/stat) 页面显示出当前应用下所有实例资源的使用情况，开发者由此可以判断实例资源是否即将或已经超限。
...
你可以在 [控制台 > 云引擎](/cloud.html?appid={{appid}}) 中点击左上角的分组选择框，点击「分组管理」，即可创建新的分组、删除分组、修改默认分组。
...
{% else %}
标准实例按天扣费，使用时间不足一天按一天收费，次日凌晨系统从账户余额中扣费，费用明细可在 [交易历史](/bill.html#/bill/cost) 中查看。
{% endif %}
