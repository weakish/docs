...
使用操作签名可以保证聊天通道的安全，这一功能默认是关闭的，可以在 [控制台 > 消息 > 即时通讯 > 设置 > 即时通讯选项](/dashboard/messaging.html?appid={{appid}}#/message/realtime/conf) 中进行开启：
...
终端->LeanMessage Cluster: 3. 将签名编码到请求中发给即时通讯服务器
LeanMessage Cluster-->终端: 4. 对请求的内容和签名进行验证，执行后续操作
```
...
「成员权限」是指将对话内成员划分成不同角色，实现类似 QQ 群管理员的效果。使用这个功能需要在 [控制台 > 消息 > 即时通讯 > 设置 > 即时通讯选项](/dashboard/messaging.html?appid={{appid}}#/message/realtime/conf) 中开启「对话成员属性功能（成员角色管理功能）」。
...
使用这个功能需要在 [控制台 > 消息 > 即时通讯 > 设置 > 即时通讯选项](/dashboard/messaging.html?appid={{appid}}#/message/realtime/conf) 中开启「黑名单功能」。
...
命中的敏感词将会被替换为 `***`。对话成员（包括发送者）会收到 `MESSAGE_UPDATE` 事件，应用可以在客户端[监听该事件](realtime-guide-intermediate.html#消息的撤回和修改)，实现相应的业务逻辑。

过滤的词库由 LeanCloud 统一提供，在 [控制台 > 消息 > 即时通讯 > 设置](/dashboard/messaging.html?appid={{appid}}#/message/realtime/conf) 中开启「消息敏感词实时过滤功能」即可使用。同时我们也支持开发者自定义敏感词词库，只需上传敏感词文件即可。
