...
LeanCloud 所有的 API 请求都通过 SSL 加密传输，保证传输过程中的数据安全性和可靠性。
LeanCloud 云端对客户端发过来的每一个请求，都进行了身份鉴别（Authentication）和访问授权（Authorization）的严格检查。
...
* 指定用户 － 指定用户才可以执行这一操作。支持通过用户名、用户的 objectId、角色名、角色的 objectId 指定用户。
...
另外，针对已经存在的 Class 你可以更新默认 ACL 和访问权限。
进入**控制台 > 存储 > 结构化数据**，选择一个 Class，再点击**权限**标签页。

以 `_` 开头的 Class，LeanCloud 提供了一些默认的基本设置。
以用于即时通讯服务的 `_Conversation` Class 为例：由于即时通讯有封装好的 SDK REST API，客户端几乎没有直接操作相应的结构化数据存储 Class 的需要，所以这个 Class 默认对所有人关闭 Class 访问权限，直接对该 Class 进行的增删查改操作仅限通过控制台或 Master Key 进行。
不过为了兼顾开发测试的便利性以及老应用的兼容性，默认设置比较宽松。
...
2. 勾选 **客户端不可见** 后，客户端发起查询的时候，返回的结果将不包含这一字段。比如，对于一个匿名发帖的应用，你仍然希望发帖的时候记录下真实的作者，但不希望将此信息返回给客户端。
...
安全中心是我们为每个应用提供的设置基本安全的入口，位置在 **控制台 > 设置 > 安全中心**。
这里可以设置或查看：
...
**控制台 > 存储 > 结构化数据 > 服务设置 > 安全设置** 中可以设置是否开启 LiveQuery。

出于安全考虑，有些应用只在服务端调用 LeanCloud 接口向用户推送通知。
对于这些应用，可以访问 **控制台 > 消息 > 推送 > 设置** 勾选 **禁止从客户端进行消息推送**。
勾选后客户端无法推送消息，只能通过 MasterKey 调用 REST API 发送或在控制台发送（**控制台 > 消息 > 推送 > 在线发送**）。

同理，有些应用只在服务端调用 LeanCloud 接口向用户发送短信。
对于这些应用，可以访问 **控制台 > 消息 > 短信 > 设置**，不勾选 **启用通用的短信验证码服务（开放 requestSmsCode 和 verifySmsCode 接口）**。
...
「Web 安全域名」域名配置策略与浏览器域安全策略一致，要求域名协议、域和端口号都严格一致，不支持子域和通配符。所以如果你要配置一个域名，要写清楚协议、域和端口，缺少一个都可能导致访问被禁止。
...
- **控制台 > 存储 > 结构化数据 > 服务设置 > 安全设置** 下可以设置 **禁止客户端创建 Class**。有些应用的开发者习惯预先规划好应用需要用到的 Class（表），并事先在控制台创建相应的 Class。对于这些开发者，推荐始终勾选这一选项。有些应用的开发者习惯直接着手开发应用的原型，在开发过程中逐渐完善数据结构。对于这些开发者，我们推荐在开发测试阶段不勾选这一选项，在应用上线前勾选这一选项。
- **控制台 > 存储 > 结构化数据 > 服务设置 > 查询设置** 下可以设置 **查询 include 引入的 Pointer 类型数据时校验 ACL 权限**。我们建议所有应用都勾选这一选项（这一选项默认处于勾选状态），以保证数据安全。

另外，**控制台 > 存储 > 结构化数据 > 用户 > 设置** 页面下有一些用户相关的安全选项，可以禁止未验证邮箱、手机的用户登录，要求修改密码时提供旧密码，密码修改后强制重新登录，验证第三方登录的 Access Token 是否有效。

## 操作日志

**控制台 > 设置 > 操作日志**会显示应用创建者及所有协作者的重要操作记录，比如删除数据操作的历史、操作用户名、操作 IP 及操作时间等，这个日志的目的是为了遇到问题更好地定位故障缘由，排查可能的恶意操作，防止应用数据被错误地改动。

## 自动备份

LeanCloud 每天自动备份应用数据，防止用户误操作删除了重要数据。
商用版应用可以在 **控制台 > 存储 > 导入导出** 恢复最近 7 天的数据（还可以指定需要恢复的 Class 或 objectId）。
开发版不支持数据恢复。

此外，开发者可以使用[数据导出功能](dashboard_guide.html#云端数据导出到本地)将应用数据备份到本地，导出功能商用版、开发版均可用。
