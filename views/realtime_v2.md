...
一个「对话」（包括普通、聊天室、系统对话）如果 1 年内没有通过 SDK 或者 REST API 发送过新的消息，或者它在 _Conversation 表中的任意字段没有被更新过，即被视为「不活跃对话」，云端会自动将其删除。（查询对话并不会更新 `_Conversation` 表，所以只查询不发送消息的对话仍会被视为不活跃对话。）
