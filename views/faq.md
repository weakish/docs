...
数据存储在执行写入操作时，大部分情况下都是即时完成的，但取决于公有云服务压力大小，偶尔可能会有延迟，不过一般来说延迟时间很短，最长在数秒。如果对一致性有严格的要求，我们也提供企业版，可以保证写入操作能立即完成。另外，取决于客户端的网络环境，SDK 发送写入操作的请求也可能存在延迟。部分 SDK 提供了异步方法，异步方法的回调函数执行意味着操作完成（或失败）。
