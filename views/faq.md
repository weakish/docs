...
数据存储在执行写入操作时，大部分情况下都是即时完成的，但取决于公有云服务压力大小，偶尔可能会有延迟，不过一般来说延迟时间很短，最长在数秒。如果对延迟完全无法容忍，请考虑使用企业版，按照您的需求独立部署存储服务，以保证写入操作能立即完成并且不会受到其它用户的干扰。另外，取决于客户端的网络环境，SDK 发送写入操作的请求也可能存在延迟。部分 SDK 提供了异步方法，异步方法的回调函数执行意味着操作完成（或失败）。
