...
这两种方式都能解决这个问题。但需要注意的是，即时通信组件 LeanMessage 也用了 AVFile 来保存消息的图片、音频等文件，并且把文件的地址写入到了消息内容中。开启了文件 SSL 域名后，历史消息中的文件地址将不会像控制台里 _File 表那样跟着改变。所以如果使用了即时通信组件并已上线，推荐使用方式一。
